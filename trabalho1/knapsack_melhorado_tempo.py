...
def avaliar(individuo, itens):
    ...
    if peso_total <= PESO_MAX:
        ...
        return valor_total - 5 * (peso_total - PESO_MAX)
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    melhor_comb, val_forca = forca_bruta(itens[:25], PESO_MAX)
    t1 = time.time()
    print(f"\nForça Bruta (15 itens): {val_forca}")
    print(f"Tempo Força Bruta: {t1 - t0:.4f} segundos")
    ...
    print(f"Tempo AG: {t3 - t2:.4f} segundos")
